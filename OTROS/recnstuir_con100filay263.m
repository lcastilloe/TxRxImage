...
%RECONSTRUCTOR DE IMAGEN CON LAS A YB Y LOS ACRCETRES EN MEDIO 
clear; clc; close all;
...
cadena_completa = strjoin(lineas, '');
...
if length(cadena_bits_puros) ~= total_bits
    error('La cantidad de bits después de eliminar a y b no es 26240. Es %d.', length(cadena_bits_puros));
end
